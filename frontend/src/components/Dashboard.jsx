...
import React, { useEffect, useState } from 'react';
import RentalHistoryItem from './RentalHistoryItem';  // For rendering each rental history item
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:4000/api/bookings/mybookings?userId=${userId}`)  // Replace with the actual endpoint to fetch rental history
      .then((response) => {
        if (!response.ok) {
  ...
  return (
    <div className="dashboard-container max-w-4xl mx-auto p-6">
      <h2 className="text-2xl font-bold text-gray-800">Your Rental History</h2>
