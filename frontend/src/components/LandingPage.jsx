...
//     let  uploadBikeButton=null;

//     if(userId){
//         uploadBikeButton =  <button onClick={()=>navigate('/add-bike')} className="bg-gradient-to-r from-blue-500 to-purple-600 hover:from-purple-600 hover:to-blue-500 text-white py-3 px-6 rounded-full shadow-lg transform transition duration-300 hover:scale-105" >Upload bikes</button>
//     }
...
//     const userId = methods.getUserByFromLS();
//     const userName = userId ? "John Doe" : "Guest"; // Replace with actual user logic.
...
//         e.preventDefault();
//         if (rating && comment) {
//             setReviews([...reviews, { name: userName, rating, text: comment }]);
//             setRating(0);
//             setComment("");
//         }
//     };
...
//                         Rent a bike and discover the freedom of the open road. Easy, affordable, and eco-friendly.
//                     </p>
//                     <div className="flex gap-x-4 mx-auto justify-center items-center ">
//                         <button
...
//     // Assuming methods.getUserByFromLS() returns user info from local storage.
//     const user = methods.getUserByFromLS();
//     const userName = user ? user.name : "Guest"; // If no user is logged in, use "Guest".
...
export default function LandingPage() {
  ...
  const userName =  localStorage.getItem('userName');
        ...
        if (rating && comment && userName) {
         ...
         axios.post('http://localhost:4000/api/reviews', {userName, rating, comment });
        
            setReviews(prevReviews => [...prevReviews, { userName: userName, rating, text: comment }]);
            setRating(0);
            setComment("");
        }
    };
    ...
    const renderReviews = () => {
        ...
        const reviewsToShow = Array.isArray(reviews) ? (showAllReviews ? reviews : reviews.slice(0, 2)) : [];
