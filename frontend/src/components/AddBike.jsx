...
const AddBike = () => {
  ...
  const handleAddBike = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:4000/api/bikes', bikeData, {
        headers: {
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-8 px-4">
      <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg shadow-lg">
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-6">Add a New Bike</h2>

        {error && (
          <div className="text-red-500 text-center mb-4">{error}</div>
        )}

        <form onSubmit={handleAddBike} className="space-y-4" encType="multipart/form-data">
          {/* Title Input */}
          <div>
